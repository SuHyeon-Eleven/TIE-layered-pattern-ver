...
class PostService {
    ...
    updatePost = async ({postId, userId, img,title,content})=>{
        ...
        const updatedPost = await this.postRepository.updatePost({
            ...
            const isUpdate = updatedPost.createdAt.toLocaleString() == updatedPost.updatedAt.toLocaleString() ? false : true
            const createdAt = dayjs(updatedPost.createdAt).format('YYYY-MM-DD HH:mm:ss')
            const updatedAt = dayjs(updatedPost.updatedAt).format('YYYY-MM-DD HH:mm:ss')
            return ({message: "게시글 수정 완료", 'post':{...updatedPost ,createdAt,updatedAt}, isUpdate })
    }
    findPostAll = async ()=>{
